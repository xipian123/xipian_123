 ...
 *
 * BoxJS Key 格式：
 *   chavy_login_sfexpress    第1个账号（chavyleung 原始订阅抓取后直接可用）
 *   chavy_login_sfexpress_2  第2个账号
 *   chavy_login_sfexpress_3  第3个账号（以此类推，最多支持10个）
 *
 * 使用方法：
 * 1. QX 添加重写规则抓取 Cookie
 *    [MITM] hostname = ccsp-egmas.sf-express.com
 *    [rewrite_local]
 *    ^https:\/\/ccsp-egmas\.sf-express\.com\/cx-app-member\/member\/app\/user\/universalSign
 *    url script-request-body https://raw.githubusercontent.com/chavyleung/scripts/master/sfexpress/sfexpress.cookie.js
 *
 * 2. 打开顺丰速运APP -> 我的顺丰 -> 任务中心 -> 去签到
 *    看到"获取会话: 成功"后，BoxJS 中会保存到 chavy_login_sfexpress
 *
 * 3. 在 BoxJS 中手动将该值复制到 chavy_login_sfexpress_1
 *    换号登录后再复制到 chavy_login_sfexpress_2，以此类推
 *
 * cron: 1 0 * * *
 */
...
!(async () => {
  // 收集所有账号：第1个读 chavy_login_sfexpress，后续读 chavy_login_sfexpress_2 ~ _10
  const accounts = []
  const keys = ['chavy_login_sfexpress', ...Array.from({length: 9}, (_, i) => `chavy_login_sfexpress_${i + 2}`)]
  ...
  if (accounts.length === 0) {
    $.msg($.name, '❌ 未找到账号', '请在 BoxJS 中配置 chavy_login_sfexpress_1')
    return
  }
  ...
  for (let i = 0; i < accounts.length; i++) {
    ...
    $.currentKey = accounts[i].key
    $.login = null
    $.sign = null
    $.tasks = []

    await loginapp(accounts[i].data)
    await $.wait(1000)
    if (!$.login || !$.login.obj) {
      $.allDesc.push(`账号${i + 1}: 登录失败`)
      continue
    }
    await loginweb()
    await $.wait(1000)
  ...
  $.msg($.name, `共 ${accounts.length} 个账号`, $.allDesc.join('\n'))
})()
  .catch((e) => $.logErr(e))
  .finally(() => $.done())
...
function collectDesc(index) {
  const success = $.sign && $.sign.success
  const lines = [`\n账号${index}:`]
  if (success) {
    if ($.sign.obj.hasFinishSign) {
      lines.push(`签到: 重复（连续${$.sign.obj.countDay}天）`)
    } else {
      lines.push(`签到: 成功（连续${$.sign.obj.countDay}天）`)
    }
  } else {
    lines.push(`签到: 失败 - ${$.sign ? $.sign.errorMessage : '未知'}`)
  }
  lines.push('每日任务:')
  for (const task of $.tasks) {
    lines.push(`  ${task.title}: ${task.result}`)
  }
  $.allDesc.push(lines.join('\n'))
}
